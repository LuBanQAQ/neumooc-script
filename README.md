...
- 修复填空题只有隐藏 input、点击后才显示富文本编辑器时无法填写的问题。
- 通过页面现有编辑器组件的方法写入内容，同步外层答案数据并校验，兼容页面拦截粘贴事件的情况。
- 批量填写按题目位置逐题激活，避免不同题型从 1 重新编号时混淆答案；失败题不计入成功数量。
- 保留分数、逗号等答案内容，支持多个空，并为无法定位控件或同步失败提供诊断。

### 本地验证

无需安装依赖，使用 Node.js 18 或以上版本运行：

```pwsh
node --check neumooc-script.user.js
node --test tests/blank-editor.test.cjs
```

回归测试模拟编辑器与答案组件的交互，不访问 AI 服务或学习平台。编辑器适配依赖平台当前的 Vue 组件结构及其暴露方法；平台升级后需要重新验证。页面数据校验通过不代表服务器已保存，使用时请切题返回确认。
